...
6. **CI 確認** — push 後に GitHub Actions の実行状況を API で確認して報告

### 除外ファイル（絶対に push しない）

- `.claude/` — ローカル設定・トークン含む（.gitignore 済み）
- `local.properties` — ANDROID_SDK パス等
- `app/testFixtures/diff/` — テスト差分画像（.gitignore 済み）
- `implementation_plan.md` — 一時的な作業メモ

### push するファイル

- `app/testFixtures/golden/*.png` — ゴールデンイメージ（回帰テストに必要）
...
GitHub Actions は、コードを push すると **自動でテストを実行** するCI/CDサービス。
パブリックリポジトリでは無料で無制限に利用可能（プライベートだと月2000分制限）。

### 自動実行される処理

| トリガー | 実行内容 | 所要時間 |
|---|---|---|
| `main` への push | JVM ユニットテスト | ~3分 |
| PR 作成 / 更新 | JVM ユニットテスト | ~3分 |
| 手動実行 (workflow_dispatch) | ユニット + Instrumented テスト | ~15分 |

### ワークフローファイル

`.github/workflows/unit-tests.yml` が CI 設定。内容:

1. **JVM ユニットテスト** (`unit-tests` ジョブ)
   - ubuntu-latest 上で JDK 17 + Flutter をセットアップ
   - `./gradlew :app:testDebugUnitTest` を実行
   - 失敗時: テストレポート + ゴールデンイメージ diff を Artifact にアップロード

2. **Instrumented テスト** (`instrumented-tests` ジョブ)
   - **手動実行のみ** (エミュレータ起動に時間がかかるため)
   - Actions タブ → "Tests" → "Run workflow" ボタンで実行

### コスト節約設定

- **concurrency**: 連続 push 時、古い実行を自動キャンセル（無駄な実行を防止）
- **timeout-minutes**: ユニット15分、Instrumented 30分で自動停止（暴走防止）
- **Instrumented テストは手動のみ**: エミュレータジョブは時間がかかるため自動実行しない
- **Gradle キャッシュ**: 2回目以降のビルドが高速化

### テスト結果の確認方法

1. https://github.com/shimabukuro256/propaint_v2 にアクセス
2. 上部の **"Actions"** タブをクリック
3. 最新のワークフロー実行をクリック
4. 緑のチェック ✓ = 成功、赤の × = 失敗
5. 失敗時は "Artifacts" セクションからテストレポートをダウンロード可能

### Claude からの CI 確認

push 後、GitHub API を使って CI の実行状況を確認する:
```bash
# 最新の実行状況を取得
curl -s "https://api.github.com/repos/shimabukuro256/propaint_v2/actions/runs?per_page=1"
```
...
上記「GitHub Actions CI (無料プラン対応)」セクションを参照。
