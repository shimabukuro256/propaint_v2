...
**注意**: propaint_flutter ディレクトリで `flutter run` を実行しないでください。これは Flutter host APK のみをビルドし、Kotlin コード（PaintFlutterActivity, MethodChannelハンドラ）を含みません。

代わりに：

```bash
# app module ディレクトリから flutter を実行
cd app  # または親ディレクトリから flutter attach
flutter run
```
